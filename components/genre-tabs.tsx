...
import { GenreCount } from "@/lib/queries/useGenreQuery";

type GenreTabsProps = {
	scrollSpy: ReturnType<typeof useGenreScrollSpy>;
	genreIds: number[];
	genreCountData: GenreCount[] | undefined;
};

export const GenreTabs = ({ scrollSpy, genreIds, genreCountData }: GenreTabsProps) => {
	...
	return (
		<div className="sticky top-16 z-40 border-b bg-background/90 backdrop-blur">
			...
			>
				{genreData.map((item) => {
					const movie_count =
						genreCountData?.find((genre) => genre.genre_id === item.id)?.movie_count ??
						0;
					return (
						<button
							key={item.id}
